...
export const hash = (input) => new Promise((res, rej) => bcrypt.hash(input.toString(), swf, (err, hash) => err ? rej(err) : res(hash)));
...
export const verifyJWT = (token, opt) => jwt.verify(token, JWTKey, opt);
